"""Tied real sine-cosine transition with learned trajectory selection."""
...
class RealSineTransition(nn.Module):
    """Autonomous periodic cell conditioned only by learned N context."""

    def __init__(self) -> None:
        super().__init__()
        self.norm = RMSNorm(D_MODEL)
        self.context = nn.Linear(D_MODEL, D_MODEL, bias=False)
        self.sine_phase = nn.Linear(D_MODEL, D_MODEL)
        self.cosine_phase = nn.Linear(D_MODEL, D_MODEL)
        self.out = nn.Linear(D_MODEL, D_MODEL)
        self.residual_scale = nn.Parameter(torch.empty(()))
        nn.init.uniform_(self.residual_scale, 0.15, 0.25)

    def forward(self, state: Tensor, context: Tensor) -> Tensor:
        features = self.norm(state) + self.context(context)
        periodic = torch.sin(self.sine_phase(features)) + torch.cos(self.cosine_phase(features))
        return state + self.residual_scale.tanh() * self.out(periodic)
...
class Model(nn.Module):
    ...
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.transition = RealSineTransition()
