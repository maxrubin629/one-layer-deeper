"""Correction-only attractor with explicit Picard refinement."""
...
TRAIN_SOLVER_STEPS, EVAL_SOLVER_STEPS = 3, 6
...
class PicardCorrector(nn.Module):
    """A contraction on e only; proposal is injected into every map call."""
    ...
    def fixed_point_map(self, e: Tensor, proposal: Tensor) -> Tensor:
        bounded_weight = torch.tanh(self.recurrent_weight)
        recurrent = 0.05 * F.linear(e, bounded_weight) / CORRECTION_WIDTH
    ...
    def forward(self, p: Tensor, h: Tensor, c: Tensor, iterations: int) -> Tensor:
        ...
        for _ in range(iterations):
            e = self.fixed_point_map(e, proposal)
        return e
...
class Model(nn.Module):
    ...
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.orbit_transition, self.correction = OrbitTransition(), PicardCorrector()
