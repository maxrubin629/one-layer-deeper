...
def masked_workspace_summary(states: Tensor, valid: Tensor) -> Tensor:
    weights = valid.to(dtype=states.dtype).unsqueeze(-1)
    count = weights.sum(dim=1).clamp_min(1.0)
    return (states * weights).sum(dim=1) / count
...
class Model(nn.Module):
    ...
    def forward(self, input_ids: Tensor, attention_mask: Tensor | None = None) -> tuple[Tensor, None]:
        ...
        for loop_index in range(loop_count):
            ...
            if loop_index > 0 and WORKSPACE_FEEDBACK_SCALE != 0.0:
                feedback_context = context + WORKSPACE_FEEDBACK_SCALE * masked_workspace_summary(workspace, valid)
            orbit = self.orbit_transition(orbit, feedback_context)
            workspace_update = (
