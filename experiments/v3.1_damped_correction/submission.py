"""Factored orbit with a damped oscillatory correction state."""
...
class DampedCorrection(nn.Module):
    def __init__(self) -> None:
        ...
        self.from_orbit = nn.Linear(ORBIT_WIDTH, CORRECTION_WIDTH, bias=False)
        self.from_workspace = nn.Linear(D_MODEL, CORRECTION_WIDTH, bias=False)
        self.from_context = nn.Linear(D_MODEL, CORRECTION_WIDTH, bias=False)
    ...
    def forward(self, e: Tensor, p: Tensor, h: Tensor, c: Tensor) -> Tensor:
        real, imag = e.chunk(2, dim=-1)
        angle = self.angle(c)
        cosine, sine = torch.cos(angle), torch.sin(angle)
        rotated = torch.cat((real * cosine - imag * sine, real * sine + imag * cosine), dim=-1)
        rho = torch.cat((self.correction_rho, self.correction_rho), dim=-1)
