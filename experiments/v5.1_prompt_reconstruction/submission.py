"""Sequence smooth-max candidate with prompt reconstruction."""
...
class Model(nn.Module):
    ...
    def forward(self, input_ids: Tensor, attention_mask: Tensor | None = None) -> tuple[Tensor, dict[str, Tensor]]:
        ...
        return logits, {"prompt_logits": self.head(normalized), "prompt_ids": input_ids, "prompt_mask": prompt_mask}
...
def training_loss(batch: TokenLossBatch) -> Tensor:
    ...
    if not isinstance(auxiliary, dict):
        raise TypeError("prompt reconstruction requires model auxiliary tensors")
    ...
    return smoothmax_primary(batch) + 0.01 * prompt_loss
