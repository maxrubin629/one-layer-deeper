"""Parameter-matched complex bilinear control with conjugate dependence."""
...
class ConjugateBilinearCore(nn.Module):
    """Strict control core: the second factor receives the conjugated input."""
...
class ConditionedBilinearTransition(nn.Module):
    """Preconditions state and context outside the conjugate control core."""

    def __init__(self) -> None:
        super().__init__()
        self.norm = RMSNorm(D_MODEL)
        self.context = nn.Linear(D_MODEL, D_MODEL, bias=False)
        self.core = ConjugateBilinearCore()
