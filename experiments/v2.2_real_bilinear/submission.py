"""Tied learned real bilinear transition."""
...
class RealBilinearTransition(nn.Module):
    """Generic learned product; it contains no task-specific arithmetic."""
    ...
    def forward(self, state: Tensor, context: Tensor) -> Tensor:
        ...
        product = self.left(features) * self.right(features)
        return state + self.residual_scale.tanh() * self.out(product)
...
class Model(nn.Module):
    ...
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.transition = RealBilinearTransition()
