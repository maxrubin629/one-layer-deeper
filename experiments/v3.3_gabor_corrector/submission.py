"""Factored orbit with a localized periodic correction activation."""
...
class GaborCorrection(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        half = CORRECTION_WIDTH // 2
        self.logit_rho = nn.Parameter(torch.zeros(half))
        self.angle = nn.Linear(D_MODEL, half)
        self.drive = nn.Linear(ORBIT_WIDTH + D_MODEL + D_MODEL, CORRECTION_WIDTH)
        self.frequency = nn.Linear(D_MODEL, CORRECTION_WIDTH)
        self.phase = nn.Linear(D_MODEL, CORRECTION_WIDTH)
        self.envelope_rate = nn.Linear(D_MODEL, CORRECTION_WIDTH)
    ...
    def forward(self, e: Tensor, p: Tensor, h: Tensor, c: Tensor) -> Tensor:
        ...
        drive = torch.tanh(self.drive(torch.cat((p, h, c), -1)))
        frequency = 0.5 + F.softplus(self.frequency(c))
        phase = self.phase(c)
        rate = F.softplus(self.envelope_rate(c))
        proposal = torch.exp(-rate * drive.square()) * torch.sin(frequency * drive + phase)
        return rho * rotated + (1.0 - rho) * proposal
...
class Model(nn.Module):
    ...
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.orbit_transition, self.correction = OrbitTransition(), GaborCorrection()
