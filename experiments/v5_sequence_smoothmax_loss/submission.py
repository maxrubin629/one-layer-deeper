"""Factored recurrent candidate with sequence-level smooth-maximum loss."""
...
def sequence_smoothmax(batch: TokenLossBatch) -> Tensor:
    token_temperature, batch_temperature = 0.25, 0.5
    losses = F.cross_entropy(batch.logits.transpose(1, 2), batch.labels.clamp_min(0), reduction="none")
    valid = batch.valid_mask.bool()
    masked = losses.masked_fill(~valid, -torch.inf)
    counts = valid.sum(1).clamp_min(1)
    per_sequence = token_temperature * torch.logsumexp(masked / token_temperature, dim=1) - token_temperature * counts.log()
    rows = valid.any(1)
    selected = per_sequence[rows]
    return batch_temperature * torch.logsumexp(selected / batch_temperature, dim=0) - batch_temperature * selected.new_tensor(selected.numel()).log()
...
SUBMISSION = Submission(build_model=build_model, build_optimizer=build_optimizer, token_training_loss=sequence_smoothmax)
