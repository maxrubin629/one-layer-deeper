"""Paired-real complex sine recurrence with no native-complex state."""
...
class ComplexLinear(nn.Module):
    """Complex-linear map implemented with shared paired-real weights."""
...
class ComplexSineTransition(nn.Module):
    """Normalization is outside the entire complex sine core."""

    def __init__(self) -> None:
        super().__init__()
        self.norm = RMSNorm(D_MODEL)
        self.context = nn.Linear(D_MODEL, D_MODEL, bias=False)
        self.phase = ComplexLinear()
    ...
    def forward(self, state: Tensor, context: Tensor) -> Tensor:
        ...
        phase_real, phase_imag = self.phase(real, imag)
        sine_real = torch.sin(phase_real) * torch.cosh(phase_imag)
        sine_imag = torch.cos(phase_real) * torch.sinh(phase_imag)
        update_real, update_imag = self.out(sine_real, sine_imag)
        update = torch.cat((update_real, update_imag), -1)
...
class Model(nn.Module):
    ...
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.transition = ComplexSineTransition()
