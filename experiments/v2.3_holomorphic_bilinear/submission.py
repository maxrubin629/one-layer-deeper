"""Paired-real complex bilinear recurrence using only complex-linear factors."""
...
class ComplexLinear(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.real = nn.Linear(COMPLEX_WIDTH, COMPLEX_WIDTH)
        self.imag = nn.Linear(COMPLEX_WIDTH, COMPLEX_WIDTH)

    def forward(self, real: Tensor, imag: Tensor) -> tuple[Tensor, Tensor]:
        out_real = F.linear(real, self.real.weight) - F.linear(imag, self.imag.weight)
        out_imag = F.linear(imag, self.real.weight) + F.linear(real, self.imag.weight)
        return out_real + self.real.bias, out_imag + self.imag.bias


class HolomorphicBilinearCore(nn.Module):
    """Strict complex core: complex-linear factors and a complex product only."""

    def __init__(self) -> None:
        super().__init__()
        self.left = ComplexLinear()
        self.right = ComplexLinear()
        self.out = ComplexLinear()

    def forward(self, features: Tensor) -> Tensor:
        real, imag = features.chunk(2, -1)
        left_real, left_imag = self.left(real, imag)
        right_real, right_imag = self.right(real, imag)
        product_real = left_real * right_real - left_imag * right_imag
        product_imag = left_real * right_imag + left_imag * right_real
        update_real, update_imag = self.out(product_real, product_imag)
        return torch.cat((update_real, update_imag), -1)


class ConditionedBilinearTransition(nn.Module):
    """Preconditions state and context outside the strict complex core."""

    def __init__(self) -> None:
        super().__init__()
        self.norm = RMSNorm(D_MODEL)
        self.context = nn.Linear(D_MODEL, D_MODEL, bias=False)
        self.core = HolomorphicBilinearCore()
        self.residual_scale = nn.Parameter(torch.empty(()))
        nn.init.uniform_(self.residual_scale, 0.075, 0.125)

    def forward(self, state: Tensor, context: Tensor) -> Tensor:
        features = self.norm(state) + self.context(context)
        return state + self.residual_scale.tanh() * self.core(features)
...
class Model(nn.Module):
    ...
    train_loops, eval_loops = TRAIN_LOOPS, EVAL_LOOPS
    ...
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.transition = ConditionedBilinearTransition()
    ...
    def forward(self, input_ids: Tensor, attention_mask: Tensor | None = None) -> tuple[Tensor, None]:
        ...
        trajectory = [state]
        loops = self.train_loops if self.training else self.eval_loops
        for _ in range(loops):
            state = self.transition(state, context)
