"""Shared autonomous GELU recurrence with learned trajectory selection."""
...
def field_masks(input_ids: Tensor, attention_mask: Tensor | None) -> tuple[Tensor, Tensor, Tensor]:
    ...
    seen_n = input_ids.eq(N_TOKEN).cumsum(dim=1).gt(0)
    seen_x = input_ids.eq(X_TOKEN).cumsum(dim=1).gt(0)
    seen_t = input_ids.eq(T_TOKEN).cumsum(dim=1).gt(0)
    seen_answer = input_ids.eq(ANS_TOKEN).cumsum(dim=1).gt(0)
    return (
        digits & seen_n & ~seen_x,
        digits & seen_x & ~seen_t,
        digits & seen_t & ~seen_answer,
    )
...
class GELUTransition(nn.Module):
    """Autonomous transition: neither depth query nor loop index is accepted."""
...
class TrajectoryReadout(nn.Module):
    ...
    def forward(self, trajectory: Tensor, query: Tensor, token_states: Tensor) -> Tensor:
        ...
        scores = (keys * query.unsqueeze(1)).sum(dim=-1) / math.sqrt(D_MODEL)
        selected = (scores.softmax(dim=1).unsqueeze(-1) * trajectory).sum(dim=1)
...
class Model(nn.Module):
    ...
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.transition = GELUTransition()
    ...
    def forward(self, input_ids: Tensor, attention_mask: Tensor | None = None) -> tuple[Tensor, None]:
        ...
        logits = self.readout(torch.stack(trajectory, dim=1), query, token_states)
        return logits, None
