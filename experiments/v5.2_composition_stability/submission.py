"""Smooth-max and reconstruction with unsupervised composition stability."""
...
class DampedCorrection(nn.Module):
    ...
    def forward(self, e: Tensor, p: Tensor, h: Tensor, c: Tensor) -> Tensor:
        ...
        return self.correction_rho * e + (1.0 - self.correction_rho) * target
...
class Model(nn.Module):
    ...
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.skip_norm, self.skip_orbit = RMSNorm(ORBIT_WIDTH), nn.Linear(ORBIT_WIDTH + D_MODEL, ORBIT_WIDTH)
    ...
    def forward(self, input_ids: Tensor, attention_mask: Tensor | None = None) -> tuple[Tensor, dict[str, Tensor]]:
        ...
        two_step = self.orbit_transition(self.orbit_transition(p, c), c)
        skip = p + 0.25 * torch.tanh(self.skip_orbit(torch.cat((self.skip_norm(p), c), -1)))
        composition_loss = F.mse_loss(skip, two_step)
        trajectory = [torch.cat((p, e), -1)]
        norm_growth = p.new_zeros(())
        loops = TRAIN_LOOPS if self.training else EVAL_LOOPS
        for _ in range(loops):
            previous_p = p
            p = self.orbit_transition(p, c)
            previous_norm = previous_p.float().norm(dim=-1).clamp_min(1e-6)
            next_norm = p.float().norm(dim=-1)
            norm_growth = norm_growth + F.relu(next_norm / previous_norm - 1.5).square().mean()
            e = self.correction(e, p, masked_mean(h, dynamics_mask), c)
            h = h + 0.25 * torch.tanh(self.workspace_local(self.workspace_norm(h)) + self.orbit_to_workspace(p)[:, None] + self.correction_to_workspace(e)[:, None] + self.context_to_workspace(c)[:, None])
        ...
        normalized = self.final_norm(h)
        logits = self.head(self.final_norm(h + selected[:, None]))
        prompt_mask = torch.ones_like(input_ids, dtype=torch.bool) if attention_mask is None else attention_mask.bool()
        auxiliary = {"prompt_logits": self.head(normalized), "prompt_ids": input_ids, "prompt_mask": prompt_mask, "composition_loss": composition_loss, "norm_growth": norm_growth / loops}
        return logits, auxiliary


def smoothmax_primary(batch: TokenLossBatch) -> Tensor:
    losses = F.cross_entropy(batch.logits.transpose(1, 2), batch.labels.clamp_min(0), reduction="none")
    valid = batch.valid_mask.bool()
    counts = valid.sum(1).clamp_min(1)
    per_sequence = 0.25 * torch.logsumexp(losses.masked_fill(~valid, -torch.inf) / 0.25, 1) - 0.25 * counts.log()
    selected = per_sequence[valid.any(1)]
    return 0.5 * torch.logsumexp(selected / 0.5, 0) - 0.5 * selected.new_tensor(selected.numel()).log()


def training_loss(batch: TokenLossBatch) -> Tensor:
    auxiliary = batch.auxiliary
    if not isinstance(auxiliary, dict):
        raise TypeError("composition loss requires model auxiliary tensors")
    prompt_loss = F.cross_entropy(auxiliary["prompt_logits"][auxiliary["prompt_mask"]].float(), auxiliary["prompt_ids"][auxiliary["prompt_mask"]])
    return smoothmax_primary(batch) + 0.01 * prompt_loss + 0.01 * auxiliary["composition_loss"] + 0.001 * auxiliary["norm_growth"]
...
SUBMISSION = Submission(build_model=build_model, build_optimizer=build_optimizer, token_training_loss=training_loss)
